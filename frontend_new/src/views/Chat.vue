  ...
  <div class="chat-container">
    ...
    <header class="chat-header">
      ...
      <div class="header-actions">
        ...
        <button class="btn btn-ghost" @click="testAudioProcessing" title="测试音频处理">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor">
            <path d="M11 5L6 9H2v6h4l5 4V5z"></path>
            <path d="M19.07 4.93a10 10 0 0 1 0 14.14M15.54 8.46a5 5 0 0 1 0 7.07"></path>
          </svg>
        ...
        >
          ...
          <template v-else>
            ...
            <div class="message-content">
              <div class="message-bubble" :class="{ 'voice-message': message.isVoice, 'text-message': !message.isVoice }">
                <!-- AI消息：显示转换功能 -->
                <template v-if="message.type === 'ai'">
                  <!-- AI文字消息 -->
                  <div v-if="!message.isVoice" class="text-content">
                    <p>{{ message.content }}</p>
                    <button 
                      class="action-button tts-button"
                      @click="playBackendAudio(message)"
                      :disabled="isConvertingTTS"
                      title="播放语音"
                    >
                      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor">
                        <path d="M11 5L6 9H2v6h4l5 4V5z"></path>
                  ...
                  <div v-else class="voice-content">
                    ...
                    <button 
                      class="action-button stt-button"
                      @click="showBackendText(message)"
                      :disabled="isConvertingSTT"
                      title="显示文字"
                    >
                      <span class="text-icon">文</span>
                    </button>
                  </div>
                </template>
                ...
                <template v-else>
                  <!-- 用户文字消息 -->
                  <div v-if="!message.isVoice" class="user-text-content">
                    <p>{{ message.content }}</p>
                  </div>
                  
                  <!-- 用户语音消息 -->
                  <div v-else class="user-voice-content">
                    <button 
                      class="play-button"
                      @click="toggleVoicePlayback(message)"
                    >
...
import { testBackendConnection, mediaAPI, sessionAPI, testBase64AudioDecoding, testAudioToBase64Conversion, testAudioSendingCompatibility, testAudioFormatChange } from '../services/api'
...
export default {
  ...
  setup(props) {
    ...
    const isConvertingTTS = ref(false)
    const isConvertingSTT = ref(false)
    ...
    onMounted(async () => {
      try {
        ...
        chatStore.setCurrentCharacter(props.characterId, props.sessionId)
    ...
    watch(() => props.characterId, async (newCharacterId) => {
      try {
        chatStore.setCurrentCharacter(newCharacterId, props.sessionId)
      } catch (error) {
      ...
      try {
        ...
        if (message.audioData) {
          console.log('播放base64音频数据:', {
            ...
            format: message.audioFormat || 'mp3'
          })
          await voiceChatManager.playBase64Audio(
            message.audioData, 
            message.audioFormat || 'mp3',
            {
              volume: voiceOutputEnabled.value ? 1 : 0
            }
          )
    ...
    const testAudioProcessing = async () => {
      try {
        console.log('开始测试音频处理功能')
        
        // 测试音频格式更改
        const formatResult = testAudioFormatChange()
        
        // 测试音频发送格式兼容性
        const compatibilityResult = await testAudioSendingCompatibility()
        
        // 测试音频文件转Base64
        const conversionResult = await testAudioToBase64Conversion()
        
        // 测试音频解码
        const decodingResult = await testBase64AudioDecoding()
        
        if (formatResult && compatibilityResult && conversionResult && decodingResult) {
          // 添加一个测试消息到聊天中
          ...
          scrollToBottom()
          console.log('测试音频消息已添加到聊天中')
          
          // 显示成功提示
          alert('音频处理测试成功！\n- 音频格式已更改为.mp3\n- 发送格式兼容性正常\n- Base64转换功能正常\n- 音频解码功能正常\n- 已添加测试消息到聊天中')
        } else {
          alert('音频处理测试失败，请查看控制台了解详情。')
        }
        
      } catch (error) {
        console.error('测试音频处理失败:', error)
        alert('测试音频处理失败: ' + error.message)
      }
    }
    
    // 播放后端返回的音频
    const playBackendAudio = async (message) => {
      if (isConvertingTTS.value) return
      
      try {
        isConvertingTTS.value = true
        console.log('播放后端音频:', message)
        ...
        if (message.audioData) {
          ...
          console.log('使用音频URL播放:', message.audioUrl)
          await voiceChatManager.playAudio(message.audioUrl)
          console.log('音频URL播放完成')
          
        } else {
          console.warn('没有音频数据可供播放')
          alert('没有音频数据可供播放')
        }
      } catch (error) {
        ...
        isConvertingTTS.value = false
      }
    }
    
    // 语音转文字
    // 显示后端返回的文字内容
    const showBackendText = async (message) => {
      if (isConvertingSTT.value) return
      
      try {
        isConvertingSTT.value = true
        console.log('显示后端文字内容:', message.content)
        
        // 直接显示后端返回的文字内容
        if (message.hasTextContent && message.content) {
          // 创建一个模态框或提示来显示文字内容
          alert(`AI回复的文字内容：\n\n${message.content}`)
          console.log('后端文字内容显示完成')
        } else {
          console.warn('没有文字内容可供显示')
          alert('没有文字内容可供显示')
        }
      } catch (error) {
        console.error('显示文字失败:', error)
        alert('显示文字失败: ' + error.message)
      } finally {
        isConvertingSTT.value = false
      }
    }
    ...
    const handleRecordingComplete = async (event) => {
      ...
      try {
        console.log('语音录制完成，开始发送到后端')
        
        // 直接发送语音文件到后端
        const response = await sessionAPI.sendAudioMessage(props.sessionId, audioBlob)
        ...
        if (response.userMessage && response.assistantMessage) {
          ...
          const userMsg = {
            ...
            type: 'user',
            content: response.userMessage.text || '语音消息',
            timestamp: new Date(response.userMessage.time),
            isVoice: true,
            duration: duration,
            audioUrl: audioUrl,
            audioBlob: audioBlob
          ...
          // 处理AI回复消息 - 用户发送语音，AI回复显示为语音
          let aiAudioUrl = null
          if (response.assistantMessage.audioData) {
            // 将Base64音频数据转换为可播放的URL（备用方案）
            const audioFormat = response.assistantMessage.audioFormat || 'mp3'
            aiAudioUrl = `data:audio/${audioFormat};base64,${response.assistantMessage.audioData}`
          }
          
          const aiMsg = {
            id: response.assistantMessage.id,
            type: 'ai',
            content: response.assistantMessage.text,
            character: character.value,
            timestamp: new Date(response.assistantMessage.time),
            isVoice: true, // 用户发送语音，AI回复显示为语音
            duration: response.assistantMessage.audioDuration || 3,
            audioUrl: aiAudioUrl,
            audioData: response.assistantMessage.audioData,
            audioFormat: response.assistantMessage.audioFormat,
            audioDuration: response.assistantMessage.audioDuration,
            hasTextContent: true // 标记有文字内容可供转换
          }
          
          // 添加到conversations中
          chatStore.conversations[chatStore.currentCharacterId].push(userMsg)
          chatStore.conversations[chatStore.currentCharacterId].push(aiMsg)
          ...
          console.warn('后端响应格式异常:', response)
...
.message {
  ...
  .message-content {
    ...
    .message-bubble {
      ...
      &.voice-message {
        padding: $spacing-sm;
        border-left: 3px solid rgba(59, 130, 246, 0.6); // 蓝色边框表示语音消息
      ...
      &.text-message {
        border-left: 3px solid rgba(34, 197, 94, 0.6); // 绿色边框表示文字消息
