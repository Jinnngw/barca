...
export const useChatStore = defineStore('chat', {
  ...
  actions: {
    ...
    // 设置当前角色和会话ID
    setCurrentCharacter(characterId, sessionId) {
      ...
      console.log('设置当前角色:', characterId, '会话ID:', sessionId)
    },
    ...
    async generateAIResponse(character, userMessage, voiceOutputEnabled = true) {
      ...
      try {
        ...
        if (response.userMessage && response.assistantMessage) {
          ...
          // 处理AI回复消息 - 用户发送文字，AI回复显示为文字但有音频数据
          let aiAudioUrl = null
          ...
          const aiMsg = {
            ...
            isVoice: false, // 用户发送文字，AI回复显示为文字
            audioUrl: aiAudioUrl,
            ...
            hasAudioData: !!response.assistantMessage.audioData // 标记有音频数据可供播放
          }
          
          // 直接添加到conversations中，避免重复添加用户消息
          if (!this.conversations[this.currentCharacterId].find(msg => msg.id === userMsg.id)) {
            this.conversations[this.currentCharacterId].push(userMsg)
          }
          this.conversations[this.currentCharacterId].push(aiMsg)
